    ...
    "memory-store": tool({
      ...
      async execute(args: any, context: any) {
            ...
            transaction(() => {
              if (data.memories) {
                for (const mem of data.memories) {
                  ...
                  if (!existing) {
                    runInsert("INSERT INTO memories (content, type, scope, importance, relevance_score, access_count, tags, keywords, project_path, session_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", [mem.content, mem.type || "project", mem.scope || "project", mem.importance || 5, mem.relevance_score ?? 0.5, mem.access_count ?? 0, mem.tags || "", mem.keywords || "", mem.project || projectPath, context.sessionID])
                    importedMemories++
                  }
                }
              }
        ...
        const memoryId = runInsert(
          ...
          [content, type, scope, importance, context.sessionID, importance * 0.1, String(args.keywords ?? ""), allTags, projectPath, embedding]
        )
