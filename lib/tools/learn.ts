...
import { getOne, getAll, execSingle, runInsert, now, saveDb } from "../db"
...
export function createLearnTool(ctx: ToolContext) {
    ...
    "memory-learn": tool({
      ...
      async execute(args: any) {
        ...
        if (mode === "arc") {
          ...
          runInsert("INSERT INTO conversation_arcs (session_id, intent, topics, message_count, project_path) VALUES (?, ?, ?, 1, ?)", [sessionId, intent, topics, projectPath])
          saveDb()
        ...
        const memoryId = runInsert("INSERT INTO memories (content, type, scope, importance, session_id, keywords, tags, project_path, embedding) VALUES (?, ?, ?, 6, ?, ?, ?, ?, ?)", [fact, subjectType, scope, sessionId, subject, autoTags, projectPath, embedding])
