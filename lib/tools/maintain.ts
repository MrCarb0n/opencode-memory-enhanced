...
import { truncate, cosineSimilarity } from "../utils"
          ...
          const dedupLoop = () => {
            for (let i = 0; i < projectMemories.length; i++) {
              for (let j = i + 1; j < projectMemories.length; j++) {
                ...
                const sim = cosineSimilarity(projectMemories[i].content, projectMemories[j].content)
                if (sim > threshold && projectMemories[i].importance >= projectMemories[j].importance) {
                  if (!dryRun) {
                    execSingle("UPDATE memories SET access_count = access_count + ?, importance = MAX(importance, ?), last_accessed = ? WHERE id = ?", [projectMemories[j].access_count, projectMemories[j].importance, now(), projectMemories[i].id])
                    execSingle("DELETE FROM memories WHERE id = ?", [projectMemories[j].id])
                  }
                  merges.push(`  Merge: [${projectMemories[j].id}] "${truncate(projectMemories[j].content, 40)}" -> [${projectMemories[i].id}] (sim: ${sim.toFixed(2)})`)
                  mergeCount++
                }
              }
            }
